 ...
 * * DESCRIÇÃO GERAL
 * -------------------------------------------------------------
 * Arquivo responsável pela visualização detalhada e atualização 
 * cadastral de usuários/funcionários já existentes no sistema.
 *
 * Executa:
 * - Recuperação de dados via JOIN (usuario + cargo).
 * - Alternância entre painéis de visualização e edição (UX).
 * - Tratamento de upload e substituição de foto de perfil 
 * (Remoção automática da imagem antiga).
 * - Validação de duplicidade de documentos ignorando o ID atual.
 * - Atualização opcional de senha com novo hash criptográfico.
 * - Controle de status da conta (Ativar/Desativar).
 * - Integração com script de exclusão de registros.
 * 
 * 
 * FLUXO DE EXECUÇÃO
 * -------------------------------------------------------------
 * 1. Valida autenticação e verifica o parâmetro 'id' via GET.
 * 2. Realiza consulta preparada para carregar dados do usuário.
 * 3. Se houver POST (toggle_status):
 * - Atualiza apenas o campo 'conta_ativa' e recarrega.
 * 4. Se houver POST (editar_usuario):
 * a. Processa nova imagem: se válida, exclui a antiga do 
 * servidor e gera novo nome único.
 * b. Sanitiza strings e remove máscaras de documentos.
 * c. Verifica se CPF/RG/Email já pertencem a OUTRO usuário.
 * d. Constrói a query de UPDATE dinamicamente (com ou sem 
 * alteração de senha).
 * e. Executa a persistência via Prepared Statement.
 * 5. Renderiza a interface: funções de formatação (PHP) tratam 
 * a exibição de documentos para o usuário.
 *
 *
 * SEGURANÇA
 * -------------------------------------------------------------
 * - Uso obrigatório de Prepared Statements para evitar SQL 
 * Injection.
 * - Proteção contra XSS na exibição de dados via lógica de 
 * output seguro.
 * - Validação de tipos de arquivos permitidos no upload.
 * - Verificação de existência física de arquivos (file_exists) 
 * antes de operações de disco (unlink).
 * - Conversão de IDs para inteiros (intval) em operações 
 * críticas.
 *
 *
 * ACESSIBILIDADE E UX
 * -------------------------------------------------------------
 * - Alternância de painéis (Visualização/Edição) via JavaScript 
 * sem recarregar a página (CSS .hidden).
 * - Funções internas de formatação dinâmica (CPF, RG, CEP, TEL) 
 * para melhor legibilidade.
 * - Badges visuais coloridos para indicação de status 
 * (Ativo/Inativo).
 * - Preview em tempo real da nova foto selecionada.
 * - Botões de ação contextualizados (Ativar vs Desativar).
 *
 *
 * DEPENDÊNCIAS
 * -------------------------------------------------------------
 * - "../../BD/conexao.php": Conexão com o banco de dados.
 * - "../../include/verificacao.php": Controle de sessão.
 * - "../../include/navbar.php": Menu de navegação.
 * - "imask": Biblioteca para máscaras de entrada no formulário.
 * - "../../assets/js/script.js": Lógica de interface e preview.
 *
 *
 * TABELAS UTILIZADAS
 * -------------------------------------------------------------
 * 1. usuario
 * - SELECT, UPDATE e DELETE (via redirecionamento).
 *
 * 2. cargo
 * - SELECT (para popular o campo de seleção de cargos).
 *
 *
 * BOAS PRÁTICAS APLICADAS
 * -------------------------------------------------------------
 * - Modularização de máscaras de exibição em funções PHP puras.
 * - Gestão eficiente de armazenamento 
 * (limpeza de arquivos órfãos).
 * - Uso de Prepared Statements com bind_param para tipos mistos.
 * - Tratamento de erros de banco de dados com blocos Try-Catch.
 * - Redirecionamento (header Location) após processamento POST 
 * para evitar reenvio de formulário.
 *
 * * -------------------------------------------------------------
 * Data: 07/03/2026
 * Versão: 1.0
