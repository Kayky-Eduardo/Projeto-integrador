document.addEventListener("DOMContentLoaded", () => {
    ...
    async function carregarJornadas() {
        ...
        const res = await fetch("../../api/api_jornada.php?acao=get_tempo");
        const json = await res.json();

        json.dados.forEach(t => {
            const opt = document.createElement("option");
            opt.value = t.id_tempo;
    ...
    document.getElementById("btn-ativar").addEventListener("click", () => {
        document.getElementById("opcoes_select").classList.toggle("oculto");
    });

    document.querySelectorAll('input[name="usuarios[]"]').forEach(cb => {
        cb.addEventListener("change", () => {
            const total = document.querySelectorAll('input[name="usuarios[]"]:checked').length;
    ...
    document.getElementById("form-setor").addEventListener("submit", async (e) => {
        ...
        const id_tempo = select.value;

        const usuarios = Array.from(
            document.querySelectorAll('input[name="usuarios[]"]:checked')
        ).map(cb => parseInt(cb.value));

        if (!nome) return mostrar("Nome obrigatório", "erro");
        if (!id_tempo) return mostrar("Selecione jornada", "erro");

        try {
            const res = await fetch("../../api/api_setores.php?acao=cadastrar_setor", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    nome_setor: nome,
                    usuarios_selecionado: usuarios,
                    id_tempo: id_tempo
                })
            });
            ...
            if (json.dados.sucesso) {
                mostrar("Cadastrado com sucesso!", "sucesso");
                location.reload(); // recarrega tabela
            } else {
                mostrar(json.dados.mensagem, "erro");
            }

        } catch (err) {
            mostrar("Erro ao cadastrar", "erro");
        }
    });
